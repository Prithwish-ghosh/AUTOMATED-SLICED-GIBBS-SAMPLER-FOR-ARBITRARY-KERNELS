...
## Elliptical Slice Sampler (multivariate) for each kernel

set.seed(1)
n_iter_funnel <- 1000
draws_funnel_ess <- matrix(0, nrow = n_iter_funnel, ncol = 10)
draws_funnel_ess[1, ] <- rep(0, 10)
nEval_funnel_ess <- 0L

mu_funnel <- rep(0, 10)
Sig_funnel <- diag(10)  # supporting normal covariance

time_ess_funnel_ell <- system.time(
  for (i in 2:n_iter_funnel) {
    out_ell <- slice_elliptical_mv(
      x          = draws_funnel_ess[i - 1, ],
      log_target = function(x) funnel_log_kernel(x, sigma = 3, mu = 0),
      mu         = mu_funnel,
      Sig        = Sig_funnel,
      is_chol    = FALSE
    )
    draws_funnel_ess[i, ] <- out_ell$x
    nEval_funnel_ess <- nEval_funnel_ess + out_ell$nEvaluations
  }
)

time_ess_funnel_ell
mean(effectiveSize(draws_funnel_ess))


## ---------------------------
## 2) HYBRID ROSENBROCK (2D)
## Supporting Normal: N(mu, Sig)
## ---------------------------
set.seed(2)
n_iter_rosen <- 1000
draws_rosen_ess <- matrix(0, nrow = n_iter_rosen, ncol = 2)
draws_rosen_ess[1, ] <- c(0, 0)
nEval_rosen_ess <- 0L

mu_rosen <- c(0, 0)
Sig_rosen <- diag(2)

time_ess_rosen <- system.time(
  for (i in 2:n_iter_rosen) {
    out_ess <- slice_elliptical_mv(
      x          = draws_rosen_ess[i - 1, ],
      log_target = function(x) hybrid_rosenbrock_log_kernel_2d(x, a = 1, b = 100),
      mu         = mu_rosen,
      Sig        = Sig_rosen,
      is_chol    = FALSE
    )
    draws_rosen_ess[i, ] <- out_ess$x
    nEval_rosen_ess <- nEval_rosen_ess + out_ess$nEvaluations
  }
)

time_ess_rosen
mean(effectiveSize(draws_rosen_ess))
...
set.seed(3)
n_iter_squig <- 1000
draws_squig_ess <- matrix(0, nrow = n_iter_squig, ncol = 3)
draws_squig_ess[1, ] <- c(0, 0, 0)
nEval_squig_ess <- 0L

mu_squig <- rep(0, 3)
Sig_squig <- diag(3)

time_ess_squig <- system.time(
  for (i in 2:n_iter_squig) {
    out <- slice_elliptical_mv(
      x          = draws_squig_ess[i - 1, ],
      log_target = function(x) squiggle_log_kernel(x, a = 1.5),
      mu         = mu_squig,
      Sig        = Sig_squig,
      is_chol    = FALSE
    )
    draws_squig_ess[i, ] <- out$x
    nEval_squig_ess <- nEval_squig_ess + out$nEvaluations
  }
)

time_ess_squig
mean(effectiveSize(draws_squig_ess))
...
set.seed(4)
n_iter_ac <- 1000
draws_ac_ess <- matrix(0, nrow = n_iter_ac, ncol = 10)
draws_ac_ess[1, ] <- rep(0, 10)
nEval_ac_ess <- 0L

mu_ac <- rep(0, 10)
Sig_ac <- diag(10)

time_ess_ac <- system.time(
  for (i in 2:n_iter_ac) {
    out <- slice_elliptical_mv(
      x          = draws_ac_ess[i - 1, ],
      log_target = function(x) allen_cahn_log_kernel(x, beta = 1, a = 0.1),
      mu         = mu_ac,
      Sig        = Sig_ac,
      is_chol    = FALSE
    )
    draws_ac_ess[i, ] <- out$x
    nEval_ac_ess <- nEval_ac_ess + out$nEvaluations
  }
)

time_ess_ac
mean(effectiveSize(draws_ac_ess))