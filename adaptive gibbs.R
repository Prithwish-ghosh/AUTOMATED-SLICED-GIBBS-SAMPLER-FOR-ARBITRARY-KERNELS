...
## Overlay true density on HMC samples
library(ggplot2)
...
xs <- seq(-5, 7, length.out = 2000)
df_target <- data.frame(x = xs, dens = sapply(xs, k1))

ggplot() +
  geom_histogram(aes(x = samples_univariate_case, y = after_stat(density)),
                 bins = 50, fill = "lightblue", color = "black", alpha = 0.7) +
  geom_line(data = df_target, aes(x = x, y = dens), linewidth = 1.2, color = "red") +
  labs(title = "Samples Generated by HMC vs True Density",
       x = "theta", y = "Density") +
  theme_minimal(base_size = 12) 

hist(hmc_out$chain[1,], col = "lightblue")

acf(samples_univariate_case, lag.max = 50)
effectiveSize(samples_univariate_casepa)
...
x1_seq <- seq(-4, 4, length.out = 200)
x2_seq <- seq(-2, 6, length.out = 200)
grid_df <- expand.grid(x1 = x1_seq, x2 = x2_seq)
grid_df$level <- apply(grid_df, 1, function(row) banana_kernel(c(row["x1"], row["x2"])))

samples_df <- data.frame(x1 = draws_banana_ess[, 1], x2 = draws_banana_ess[, 2])

breaks_seq <- seq(0, 1, by = 0.05)
n_bins <- length(breaks_seq) - 1

ggplot() +
  geom_contour_filled(data = grid_df, aes(x = x1, y = x2, z = level),
                      breaks = breaks_seq) +
  scale_fill_manual(values = colorRampPalette(c("white", "steelblue", "darkred"))(n_bins)) +
  geom_point(data = samples_df, aes(x = x1, y = x2),
             color = "purple", size = 1, alpha = 0.6) +
  labs(title = "Contour Plot of Banana-Shaped Kernel with Elliptical Slice Samples",
       x = expression(x[1]), y = expression(x[2]), fill = "level") +
  theme_minimal(base_size = 12) +
